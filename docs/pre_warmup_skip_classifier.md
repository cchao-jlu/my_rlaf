...
- 生成评估配置：`configs/config_eval_guided_solver_pre_warmup_skip_classifier.yaml`
- L2：`0.5`
- threshold：`0.155104`
...
- 额外约束：允许训练集中少量非 easy 的低风险样本被 skip，但仍要求强负例零误跳。
...
```bash
python3 evaluate_guided_solver.py --config-name config_eval_guided_solver_pre_warmup_skip_classifier
```
