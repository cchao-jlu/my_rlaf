...
CANDIDATE_COMBINED = ROOT / "runs/analysis/benchmark_candidate_smoke/combined.csv"
SOURCE_ROOT = ROOT / "data/benchmark_candidates"
SUBSET_ROOT = ROOT / "data/benchmark_candidate_neural_triage"
WORK_DIR = ROOT / "runs/analysis/benchmark_candidate_neural_triage"
...
DOC_PATH = ROOT / "docs/benchmark_candidate_neural_triage.md"
...
class CandidateSubset:
    ...
    def eval_glob(self) -> str:
        return f"data/benchmark_candidate_neural_triage/{self.family}/{self.size}/*.cnf"
...
def load_both_unknown_subsets(families: set[str], sizes: set[int]) -> list[CandidateSubset]:
    frame = pd.read_csv(CANDIDATE_COMBINED)
    rows = []
    for (family, size, file_key), group in frame.groupby(["family", "size", "file_key"], sort=True):
        if family not in families or int(size) not in sizes:
            continue
        solvers = group.set_index("solver_name")["solved"].astype(bool)
        if {"march", "cadical"}.issubset(solvers.index) and (not bool(solvers["march"])) and (not bool(solvers["cadical"])):
            rows.append({"family": family, "size": int(size), "file_key": file_key})
    subset = pd.DataFrame(rows)
    if subset.empty:
        raise ValueError("No candidate both-unknown subsets matched the requested family/size filters")
    subsets = []
    for (family, size), group in subset.groupby(["family", "size"], sort=True):
        subsets.append(CandidateSubset(family=str(family), size=int(size), file_keys=tuple(group["file_key"].astype(str))))
    return subsets


def prepare_subset(subset: CandidateSubset, clean: bool = False) -> None:
...
def raw_csv_path(subset: CandidateSubset, spec: MethodSpec, seed: int) -> Path:
    return (RAW_DIR / subset.label / spec.method / f"seed{seed}.csv").resolve()


def build_command(subset: CandidateSubset, spec: MethodSpec, seed: int, workers: int) -> list[str]:
    raw_csv = raw_csv_path(subset, spec, seed)
...
def run_eval(subset: CandidateSubset, spec: MethodSpec, seed: int, workers: int, dry_run: bool, skip_existing: bool) -> Path:
    raw_csv = raw_csv_path(subset, spec, seed)
    if skip_existing and raw_csv.exists():
        print(f"skip existing {raw_csv.relative_to(ROOT)}", flush=True)
        return raw_csv
    command = build_command(subset, spec, seed=seed, workers=workers)
...
def load_raw(subset: CandidateSubset, spec: MethodSpec, seed: int, raw_csv: Path) -> pd.DataFrame:
...
def summarize(combined: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    summary_rows = []
    for (family, size, method, paper_name, seed), group in combined.groupby(["family", "size", "method", "paper_name", "seed"], sort=True):
        summary_rows.append(
            {
                "family": family,
                "size": int(size),
    ...
    for (family, size, file_key), group in combined.groupby(["family", "size", "file_key"], sort=True):
        ...
        pattern = []
        for spec in METHODS:
            method_group = group[group["method"] == spec.method]
            ...
            row[f"{spec.method}_time_mean"] = float(method_group["time"].mean())
            row[f"{spec.method}_result_values"] = ",".join(sorted(map(str, method_group["Result"].unique())))
...
def write_doc(subset_rows: pd.DataFrame, summary: pd.DataFrame, overlap: pd.DataFrame) -> None:
    aggregate = (
        ...
        .agg(seeds=("seed", "nunique"), solved_mean=("solved", "mean"), solved_min=("solved", "min"), solved_max=("solved", "max"), mean_time_mean=("mean_time", "mean"))
        .reset_index()
    )
    strict = (
    ...
    strict["online_only_strict"] = overlap.groupby(["family", "size"], sort=True).apply(
    ...
    if int(strict["online_strict"].sum()) > 0:
        lines.extend(
            [
                "- At least one candidate both-unknown subset has frozen Online-Consistent",
                "  solves. This is only a triage signal; it would require repeated",
                "  strong-solver confirmation and a larger generated benchmark before",
                "  becoming a performance claim.",
            ]
        )
    else:
        lines.extend(
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run frozen neural triage on candidate both-unknown subsets.")
    parser.add_argument("--families", nargs="+", default=["3sat"])
    parser.add_argument("--sizes", type=int, nargs="+", default=[450, 500])
    ...
    subsets = load_both_unknown_subsets(families=set(args.families), sizes=set(args.sizes))
    ...
    for subset in subsets:
        ...
        for seed in args.seeds:
            for spec in METHODS:
                raw_csv = run_eval(subset, spec, seed=seed, workers=args.workers, dry_run=args.dry_run, skip_existing=args.skip_existing)
                if not args.dry_run:
                    frames.append(load_raw(subset, spec, seed=seed, raw_csv=raw_csv))
    if args.dry_run:
