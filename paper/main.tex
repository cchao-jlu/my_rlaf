...
On 200 held-out 3SAT-400 instances, the end-to-end Local5--CaDiCaL55 portfolio solves 79--80/200 over three full repeats. However, repeated standalone CaDiCaL 60s runs solve 75, 80, and 80 instances, so the matched portfolio deltas are +4, 0, and 0. A repeated-CaDiCaL overlap audit finds three Local-solved instances that CaDiCaL misses in all repeats, two of which are already solved by the Online-Consistent Selector; the Local-only strict boundary contribution is \texttt{3sat\_188.cnf}. A further baseline audit with the existing March solver changes the interpretation: under a strict 60s wall-clock count, March solves 184/200 in each of three repeats and solves all three CaDiCaL-strict complement instances. Neural-stage results therefore explain a risk-control mechanism inside the neural-guided Glucose workflow, not a performance advantage over strong SAT baselines.
\end{abstract}
...
\begin{enumerate}
    ...
    \item We audit the resulting neural-first/CDCL-second portfolio against repeated CaDiCaL and March baselines, exposing the boundary where neural-guided Glucose improvements stop being competitive with stronger SAT solvers.
\end{enumerate}
...
We evaluate whether online-consistent conservative selection can provide a useful neural first stage for a strong CDCL portfolio under real wall-clock evaluation. The main questions are: how the end-to-end Local5--CaDiCaL55 schedule compares with standalone CaDiCaL 60s under repeated baselines, whether any apparent advantage is stable under repeated CaDiCaL runs, and how much of the portfolio-only evidence comes from neural-first complementarity rather than second-stage runtime-boundary behavior.

\subsection{Experimental Setup}

The main benchmark is \texttt{data/test/3sat/400/*.cnf}, containing 200 held-out 3SAT-400 instances. All full400 conclusions come from real wall-clock runs rather than offline estimates or stale selector caches. Standalone baselines use a nominal 60s solve limit; the external guard is 65s, so a small number of solver-reported CPU times may slightly exceed 60s. As solver-default references, unweighted Glucose solves 13/200 instances and the original standalone CaDiCaL run solves 75/200; repeated CaDiCaL 60s runs solve 75, 80, and 80 instances. We also audit the existing unweighted March binary for three full repeats. Because the current March runner uses only an external timeout, we report a strict 60s count by treating March solutions with wall time greater than 60s as timeouts.
...
Table~\ref{tab:portfolio-main} gives the main end-to-end portfolio result together with repeated standalone CaDiCaL 60s runs. The Local5--CaDiCaL55 portfolio solves 79--80/200 instances over three full400 repeats. The original CaDiCaL run solved 75/200, but repeated CaDiCaL runs solve 80/200; matched deltas are therefore +4, 0, and 0. The portfolio has zero CaDiCaL-only losses in all three repeats, but its solved-count advantage is boundary-sensitive rather than repeated-baseline robust.
...
Table~\ref{tab:portfolio-claim-split} splits the portfolio-only evidence by source. This distinction is important: the paper does not claim that neural guidance alone contributes the entire original-run +4 delta, and the repeated-CaDiCaL audit shows that only one neural-first complement remains strict across all CaDiCaL repeats. The remaining portfolio-only evidence is second-stage CaDiCaL cutoff/runtime-boundary behavior or CaDiCaL repeat variance.
...
The portfolio improvement is not uniform across all instances. Under the original CaDiCaL run, portfolio-only neural-first complement comes from \texttt{3sat\_132.cnf}, \texttt{3sat\_140.cnf}, and \texttt{3sat\_25.cnf}. Repeated CaDiCaL 60s runs solve \texttt{3sat\_132.cnf} and \texttt{3sat\_25.cnf}, leaving \texttt{3sat\_140.cnf} as the strict repeated-baseline complement within the original-run portfolio-only split. The runtime-boundary portfolio-only cases \texttt{3sat\_111.cnf} and \texttt{3sat\_48.cnf} are also solved by repeated standalone CaDiCaL 60s runs.

Table~\ref{tab:cadical-overlap} gives the full repeated-CaDiCaL overlap audit. This audit gives a broader view because it is not restricted to instances that are portfolio-only under the original CaDiCaL run. Local Boundary Correction solves three instances that CaDiCaL misses in all three 60s repeats: \texttt{3sat\_140.cnf}, \texttt{3sat\_147.cnf}, and \texttt{3sat\_188.cnf}. Online-Consistent Selector already solves \texttt{3sat\_140.cnf} and \texttt{3sat\_147.cnf}; the only strict Local-only gain beyond Online and repeated CaDiCaL is \texttt{3sat\_188.cnf}. Conversely, the five one-shot timeouts recovered by Online are all solved by CaDiCaL in all three repeats, so they explain neural-workflow recovery but not strong-CDCL complementarity.

The March baseline further limits the claim. Under the strict 60s count, March solves 184/200 instances in each of three repeats with mean capped time 26.713s, and its strict solved/unsolved status is stable on all 200 instances. It solves \texttt{3sat\_140.cnf}, \texttt{3sat\_147.cnf}, and \texttt{3sat\_188.cnf} in all three repeats, removing the CaDiCaL-strict complement set. Local Boundary Correction has no solved-count complement against repeated March strict-60. We therefore treat the neural results as evidence about risk-controlled feedback inside a neural-guided Glucose workflow, not as a strong-SAT-baseline performance result.
...
The experiments support a conservative failure-boundary interpretation. Neural feedback is not uniformly beneficial across all instances, and the neural-guided Glucose workflow alone does not dominate stronger SAT solvers. Its value in the present evidence is methodological: it shows how event traces can control unsafe neural feedback and repair a narrow class of over-conservative decisions. The Online-Consistent Selector is valuable because it makes adapter activation conditional on evidence from the actual CDCL trajectory, while keeping the final intervention sparse.
...
The portfolio result also has a sharp claim boundary. The original single-run comparison gives a +4 portfolio advantage, but repeated CaDiCaL baselines solve 80/200 and remove the matched solved-count advantage in repeats 1 and 2. Full repeated-baseline overlap has three Local-solved CaDiCaL-unsolved instances, but only one is a Local-only gain beyond Online: \texttt{3sat\_188.cnf}. March strict-60 solves all three of these instances and 184/200 overall. Other portfolio-only cases should be treated as cutoff/runtime-boundary evidence or CaDiCaL repeat variance, not neural solves. The current evidence is strongest on the full 3SAT-400 test set. Appendix results on 3SAT-300 and 3SAT-350 show that the Online-Consistent Selector does not collapse on smaller held-out sizes, and the broader appendix table includes Glucose default and Old Compact references. These remain generality and baseline robustness checks rather than a new cross-scale claim. The runtime robustness evidence is also scoped: it covers same-seed repeats and seeds 1, 2, and 3 for the neural stage plus three full portfolio and CaDiCaL repeats, not cross-machine reproducibility.

\section{Conclusion}

This paper studies event-conditioned neural SAT guidance as a conservative first stage for a CDCL portfolio. Rather than calling a neural model at every branching decision, the solver collects low-frequency CDCL event traces, uses an Online-Consistent Selector to decide whether event-adapted guidance should be activated, and applies a guarded Local Boundary Correction for a narrow class of over-conservative boundary cases. On held-out 3SAT-400 instances, the resulting Local5--CaDiCaL55 portfolio solves 79--80/200 instances over three repeats. Repeated standalone CaDiCaL 60s runs solve 75, 80, and 80 instances, so the portfolio evidence is boundary-sensitive rather than a robust solved-count win. A repeated March strict-60 audit solves 184/200 in every repeat and removes the current strong-baseline complementarity claim. The results suggest that solver feedback is most useful when treated as sparse evidence for risk-controlled neural intervention and failure-boundary analysis, not as an unconditional replacement for a strong SAT solver.
