...
DEFAULT_OUT_PREFIX = ROOT / "runs/analysis/echosat_adapter_delta_v2_iter235_canonical_order_low_warmup"
DEFAULT_DOC_PREFIX = ROOT / "docs/echosat_adapter_delta_v2_iter235_canonical_order_low_warmup"
...
def analysis_command(args: argparse.Namespace, per_instance_paths: list[Path]) -> list[str]:
    return [
        sys.executable,
        str(ROOT / "analyze_echosat_canonical_order_audit.py"),
        "--canonical-per-instance-csvs",
        *[str(path) for path in per_instance_paths],
        "--original-observations-csv",
        str(resolve(args.original_observations_csv)),
        "--canonical-observations-csv",
        str(resolve(args.out_prefix).with_name(f"{resolve(args.out_prefix).name}_sweep_observations.csv")),
        "--pair-deltas-csv",
        str(resolve(args.out_prefix).with_name("echosat_adapter_delta_v2_iter235_canonical_order_pair_deltas.csv")),
        "--summary-csv",
        str(resolve(args.out_prefix).with_name("echosat_adapter_delta_v2_iter235_canonical_order_summary.csv")),
        "--doc",
        str(resolve(args.doc_prefix).with_name("echosat_adapter_delta_v2_iter235_canonical_order_audit.md")),
    ]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run canonical DIMACS order low-warmup EchoSAT audit.")
    parser.add_argument("--checkpoint", type=Path, default=DEFAULT_CHECKPOINT)
    parser.add_argument("--manifest", type=Path, default=DEFAULT_MANIFEST)
    parser.add_argument(
        "--original-observations-csv",
        type=Path,
        default=ROOT / "runs/analysis/echosat_adapter_delta_v2_iter235_low_warmup_sweep_observations.csv",
    )
...
def main() -> None:
    ...
    if "--weighted-no-pre" in sys.argv[1:]:
        raise SystemExit("Canonical order audit is locked to patched_pretrue_main; do not pass --weighted-no-pre.")
    if not resolve(args.manifest).exists():
        raise SystemExit(
            f"Canonical manifest does not exist: {resolve(args.manifest)}. "
            "Run build_echosat_canonical_order_manifest.py first."
        )
