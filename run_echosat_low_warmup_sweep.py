...
DEFAULT_OUT_PREFIX = ROOT / "runs/analysis/echosat_adapter_delta_v2_iter235_low_warmup"
DEFAULT_DOC_PREFIX = ROOT / "docs/echosat_adapter_delta_v2_iter235_low_warmup"
DEFAULT_FAMILIES = ["complete_coloring", "php", "random_3sat_control"]
...
def analysis_command(args: argparse.Namespace, per_instance_paths: list[Path]) -> list[str]:
    ...
    return [
        sys.executable,
        str(ROOT / "analyze_echosat_low_warmup_sweep.py"),
        "--per-instance-csvs",
        *[str(path) for path in per_instance_paths],
        "--observations-csv",
        str(out_prefix.with_name(f"{out_prefix.name}_sweep_observations.csv")),
        "--overall-csv",
        str(out_prefix.with_name(f"{out_prefix.name}_sweep_overall.csv")),
        "--by-family-csv",
        str(out_prefix.with_name(f"{out_prefix.name}_sweep_by_family.csv")),
        "--by-base-csv",
        str(out_prefix.with_name(f"{out_prefix.name}_sweep_by_base.csv")),
        "--doc",
        str(doc_prefix.with_name(f"{doc_prefix.name}_sweep.md")),
        "--families",
        *args.families,
    ]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the formal EchoSAT AdapterDelta v2 low-warmup runtime sweep.",
    )
...
def main() -> None:
    args = parse_args()
    if not args.budgets:
        raise SystemExit("At least one warmup conflict budget is required.")
    if "--weighted-no-pre" in sys.argv[1:]:
        raise SystemExit("Low-warmup sweep is locked to patched_pretrue_main; do not pass --weighted-no-pre.")
